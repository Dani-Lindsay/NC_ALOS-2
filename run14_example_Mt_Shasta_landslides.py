...
eel_lat, eel_lon =  41.410029, -122.1939
...
dic_Ee = {"ORBIT_DIRECTION" : "descending", 
           "PATH" : "170", 
           "Platform" : "ALOS-2", 
           "geo_file" : "/Volumes/Seagate/NC_Landslides/Timeseries_2/y0_x0_box/geo/geo_geometryRadar.h5", 
           "vel_file" : "/Volumes/Seagate/NC_Landslides/Timeseries_2/y0_x0_box/geo/geo_velocity.h5", 
           "ts_file" :  "/Volumes/Seagate/NC_Landslides/Timeseries_2/y0_x0_box/geo/geo_timeseries_SET_ERA5_ramp_demErr.h5", 
           "vel_grd" :  "/Volumes/Seagate/NC_Landslides/Timeseries_2/y0_x0_box/geo/geo_velocity.grd", 
           }

insar_Ee = utils.load_insar_vel_ts_as_dictionary(dic_Ee)
points_Ee = [(-122.23451,  41.41083), (-122.20585,  41.40829)] # -123.48453, 40.06314, -123.47859, 40.06566, -123.47774, 40.06503
#41.41083, -122.23451
...
with fig.subplot(nrows=1, ncols=2, figsize=("12.0c", "6.25c"), autolabel="a)", sharex="l", frame=["WSrt"], margins=["0.3c", "0.3c"],):
    
    lon_step = 0.1151
    lat_step = 0.0891
    
    region = "%s/%s/%s/%s" % (points_Ee[1][0]-lon_step, points_Ee[1][0]+lon_step, points_Ee[1][1]-lat_step, points_Ee[1][1]+lat_step), # Landslide 2 Graham
    ...
    fig.basemap(frame=["wSrt", "xa0.1", "ya0.1"], region=region, projection=fig_size, panel=True)
    pygmt.makecpt(cmap="vik", series=[-0.09, 0.11])
    fig.grdimage(grid=dic_Ee['vel_grd'], cmap=True, region=region, projection=fig_size)
    fig.plot(data=landslide_poly_file, pen="1.0p,white", projection=fig_size)
    
    
    
    fig.text(region=region, projection=fig_size, text="Mt. Shasta", position="TC",offset ="0.0/-0.2c") 
    
    
    labels = ["g)", "h)"]
    for (lon, lat), lbl in zip(points_Ee, labels):
        # Plot marker at (lon, lat)
        fig.plot(x=lon, y=lat, style="c.2c", pen="0.8p,blacl", region=region, projection=fig_size)
    ...
    df = pd.DataFrame(
        data={
            "x": [points_Ee[1][0]+lon_step*0.9,points_Ee[1][0]+lon_step*0.9],
            "y": [points_Ee[1][1]+lat_step*0.9, points_Ee[1][1]+lat_step*0.9],
...
########3 Time series Eel 
with fig.subplot(nrows=2, ncols=1, figsize=("8.5c", "6.25c"), autolabel="c)", sharex="b", frame=["lSEt" , "xaf", "ya+l LOS (mm/yr)"],
                 margins=["0.05c", "0.05c"],
):
    region_ts = [t0-0.1, te+0.1, -15, 5]
    y_text = -12
    ...
    # fig.plot(x=[eq1, eq1], y=[-12, 5],  pen="1p,black,--",) 
    # fig.plot(x=[eq2, eq2], y=[-12, 5],  pen="1p,black,--",) 
    # fig.text(text="M6.4", x=eq1, y=0, justify="LM", offset="0.1c/0.0c",)
    # fig.text(text="M6.4", x=eq2, y=0, justify="LM", offset="0.1c/0.0c",)
    fig.plot(x=[vel_t1, vel_t2], y=[y_text+2, y_text+2],  pen="2p,darkorange", region=region_ts, transparency=50)
    fig.plot(x=[vel_t3, vel_t4], y=[y_text, y_text],  pen="2p,darkorange", region=region_ts, transparency=50)
    fig.text(text=f"{vel_Ee_g_22:.1f} ± {err_Ee_g_22:.1f} cm/yr", x=(vel_t1+vel_t2)/2, y=y_text+2, font="8p,Helvetica,black", offset="0c/0.2c")
    ...
    fig.plot(x=[vel_t1, vel_t2], y=[y_text+2, y_text+2],  pen="2p,darkorange", region=region_ts, transparency=50)
    fig.plot(x=[vel_t3, vel_t4], y=[y_text, y_text],  pen="2p,darkorange", region=region_ts, transparency=50)
    fig.text(text=f"{vel_Ee_h_22:.1f} ± {err_Ee_h_22:.1f} cm/yr", x=(vel_t1+vel_t2)/2, y=y_text+2, font="8p,Helvetica,black", offset="0c/0.2c")
